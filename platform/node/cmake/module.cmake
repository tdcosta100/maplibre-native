...
function(add_node_module NAME)
    ...
    foreach(_ABI IN LISTS _ABIS)
        ...
        # Download the win-x86 libraries if we are compiling on Windows and don't have them yet
        if((MSVC OR MINGW) AND NOT EXISTS "${_CACHE_DIR}/lib/node/${_NODE_VERSION}/win-x86/node.lib")
            _node_module_download(
                "win-x86 library for Node ${_NODE_VERSION}"
                "https://nodejs.org/download/release/${_NODE_VERSION}/win-x86/node.lib"
                "${_CACHE_DIR}/lib/node/${_NODE_VERSION}/win-x86/node.lib"
            )
        endif()

        # Download the win-x64 libraries if we are compiling on Windows and don't have them yet
        if((MSVC OR MINGW) AND NOT EXISTS "${_CACHE_DIR}/lib/node/${_NODE_VERSION}/win-x64/node.lib")
            _node_module_download(
                "win-x64 library for Node ${_NODE_VERSION}"
                "https://nodejs.org/download/release/${_NODE_VERSION}/win-x64/node.lib"
                "${_CACHE_DIR}/lib/node/${_NODE_VERSION}/win-x64/node.lib"
            )
        endif()
        ...
        if(MSVC OR MINGW)
            if(CMAKE_SIZEOF_VOID_P EQUAL 8)
                target_link_libraries(${_TARGET} PRIVATE "${_CACHE_DIR}/lib/node/${_NODE_VERSION}/win-x64/node.lib")
            else()
                target_link_libraries(${_TARGET} PRIVATE "${_CACHE_DIR}/lib/node/${_NODE_VERSION}/win-x86/node.lib")
            endif()
        endif()
